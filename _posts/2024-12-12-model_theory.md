...
> **定理**  Los-Vaught
> 假设 $$T$$ 是 $$\kappa$$-范畴的，其中 $$\kappa \geq \vert \mathcal{L}\vert $$，那么：
> 1. 任意两个无限模型都是初等等价的。
...
> $$M$$，则对于任意无穷基数 $$\kappa \geq \vert  \mathcal{L} \vert $$，存在
> $$\mathcal{L}$$ 的一个结构 $$N$$ 使得 $$\vert  N \vert  = \kappa$$，且：
> 1. 如果 $$\kappa < \vert M \vert  $$ 则 $$N$$ 是 $$M$$ 的初等子结构。
> 2. 如果 $$\kappa > \vert  M \vert $$ 则 $$N$$ 是 $$M$$ 的初等扩张。
...
**例**
对于 $$\kappa < \vert  \mathbb{R} \vert $$ 的基数，实有序域上的理论不是
$$\kappa$$-范畴的。
